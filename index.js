...
// I declared my variable and set it to is31Flavors and then set the operation to check if the equality of list.length and 31. If the variable is true or false then you get an output of originalFlavors
...
console.log(is31Flavors(originalFlavors)); //for testing purposes
...
console.log("Remove Flavor",removeFlavorByName(originalFlavors,"Vanilla")); //For testing
...
console.log("Copy",copy(originalFlavors));
...
function filterByWord(list, toHighlight){

    let newCopy = copy(list);
    let toReturn = [];

    for (let i = 0; i < list.length; i++){
        if (list[i].includes(toHighlight)){
            newCopy.splice(i,1);
